...
st.set_page_config(layout="wide", page_icon="💬", page_title="KB AI - Tables")
...
if not user_api_key:
    ...
    uploaded_files = utils.handle_upload(["csv", "xlsx"])
    ...
    if uploaded_files:
        ...
        if uploaded_file.type == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" or \
        ...
        if submitted_query:
            ...
            result, captured_output, image = csv_agent.get_agent_response(df, query)
            #csv_agent.display_agent_response(result, image)
            cleaned_thoughts = csv_agent.process_agent_thoughts(captured_output)
            csv_agent.display_agent_thoughts(cleaned_thoughts)
            csv_agent.update_chat_history(query, result, image)
